			...
			<div class="col-12">
				<div class="card">
					<div class="card-header bg-light">
						<h3 class="card-title"><i class="fa fa-list text-blue"></i> Data Balita</h3>
						<div class="text-right">
							<button type="button" class="btn btn-sm btn-outline-primary" onclick="add_balita()"
								title="Add Data"><i class="fas fa-plus"></i> Add</button>
						</div>
					</div>
					<!-- /.card-header -->
					<div class="card-body">
						<table id="tb_balita" class="table table-bordered table-striped table-hover">
							<thead>
								<tr class="bg-info">
									<th>NIK</th>
									<th>Nama Balita</th>
...
<div class="modal fade" id="modal-default">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title ">View Balita</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		//datatables
		table = $("#tb_balita").DataTable({
			"responsive": true,
			"autoWidth": false,
			"language": {
				"sEmptyTable": "Data Balita Kosong"
			},
			...
			"ajax": {
				"url": "<?php echo site_url('balita/ajax_list')?>",
				"type": "POST"
			},
			...
			"columnDefs": [{
					...
					"render": function (data, type, row) {
						return "<a class=\"btn btn-xs btn-outline-info\" href=\"javascript:void(0)\" title=\"View\" onclick=\"vbalita(" +
							row[8] +
							")\"><i class=\"fas fa-eye\"></i></a> <a class=\"btn btn-xs btn-outline-primary\" href=\"javascript:void(0)\" title=\"Edit\" onclick=\"edit_balita(" +
							row[8] +
							")\"><i class=\"fas fa-edit\"></i></a> <a class=\"btn btn-xs btn-outline-danger\" href=\"javascript:void(0)\" title=\"Delete\" nama=" +
							row[1] + "  onclick=\"delbalita(" + row[8] +
	...
	//view
	function vbalita(id) {
		$('.modal-title').text('View Balita');
		$("#modal-default").modal('show');
		$.ajax({
			url: '<?php echo base_url('balita/viewbalita '); ?>',
			type: 'post',
			data: 'table=tb_balita&id=' + id,
	...
	function delbalita(id) {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('balita/delete');?>",
					type: "POST",
					data: "id_balita=" + id,
					cache: false,
					dataType: 'json',
	...
	function add_balita() {
		...
		$('.modal-title').text('Add Balita'); // Set Title to Bootstrap modal title
	}

	function edit_balita(id) {
		save_method = 'update';
		$('#form')[0].reset(); // reset form on modals
		$('.form-group').removeClass('has-error'); // clear error class
		$('.help-block').empty(); // clear error string
		...
		$.ajax({
			url: "<?php echo site_url('balita/edit_balita')?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function (data) {

				$('[name="id_balita"]').val(data.id_balita);
				$('[name="nik_balita"]').val(data.nik_balita);
				$('[name="nama_balita"]').val(data.nama_balita);
				...
				$('.modal-title').text('Edit Balita'); // Set title to Bootstrap modal title
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('balita/insert')?>";
		} else {
			url = "<?php echo site_url('balita/update')?>";
		}
		...
		<div class="modal-content ">
			...
			<div class="modal-body form">
				<form action="#" id="form" class="form-horizontal">
					<input type="hidden" value="" name="id_balita" />
					<div class="card-body">
						<div class="form-group row ">
							...
							<div class="col-sm-9 kosong">
								<input onkeypress="return hanyaAngka(event)" type="text" class="form-control"
									name="nik_balita" id="nik_balita" placeholder="Masukan NIK">
						...
						<div class="form-group row ">
							<label for="alamat" class="col-sm-3 col-form-label">Nama Orangtua</label>
							<div class="col-sm-9 kosong">
